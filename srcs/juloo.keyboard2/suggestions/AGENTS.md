...
- Changed candidate/autocorrect replacements keep their prepared learning token pending only through the immediate-Backspace window at the exact post-replacement insertion point. A selection callback that returns to any position in the corrected word settles the pending replacement even when editor readback still reports the old insertion point. Exact immediate undo rejects the changed target, learns an unrecognized raw source literal only when no manual-correction provenance exists, and otherwise commits the prior editor-verified manual-correction fallback; the next accepted action commits the changed token once.
