...
- Short bounded horizontal swipes change pages circularly in the visible drag direction (left=next page, right=previous page); sub-threshold taps and vertical gestures remain unaffected. First↔last wraps slide smoothly: at a scroll edge the far-end page is rotated next to the visible edge when the gesture begins, so the wrapped page is a real scroll neighbour; two-page strips fall back to an instant wrap. Because rotation reorders children mid-dispatch, it must repair child bounds synchronously so the in-flight tap hit-tests the new order, and `refresh_config` must reset the scroll offset so a rebuilt strip always reopens on the first page.
