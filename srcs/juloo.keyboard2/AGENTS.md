...
- Normal-editor Backspace must verify the editor mutation after each deletion API call before falling back; an editor that deletes text but returns `false` must never cause a second deletion. Keep Termux raw `KEYCODE_DEL` behavior unchanged.
...
- Adaptive learning records editor-verified typo corrections through prepare-before-mutation commit tokens whenever Suggestions or Autocorrect is active; repeated exact pairs may cover up to two textual edits. Immediate verified correction undo rejects the changed target and learns an unrecognized raw source literal when no manual-correction provenance exists; otherwise it preserves the prior manual evidence. A learned unigram literal is user-owned vocabulary and must never be replaced by an ordinary spelling correction, although alternatives remain visible and tappable; four accepted exact choices may change only its straight-apostrophe punctuation without changing its letters.
