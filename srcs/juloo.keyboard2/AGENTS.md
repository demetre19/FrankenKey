...
- Keyboard Reader is default-off. Enabled empty readable editors show a centered, horizontally scrollable icon strip ordered Voice/Settings/AI/Read Clipboard/Library. The centered AI omnibutton (four-arrows icon) has a configurable tap plus eight configurable swipe sectors — every gesture maps to any catalog action including No action (defaults: tap Chat, up Summary One, up-right Summary Two, right Quiz, down-right Share, down Saved, down-left Speed Read, left AI settings, up-left Chat) editable with reset from the Settings AI button row. The quick surface keeps the loaded source text visible at all times above the output and surfaces full provider errors in the output area. Input/candidates hide the strip. Done/Enter retains top-left voice typing. Explicit playback replaces candidates with title, transport, and full-width WPM slider. Strip visibility re-evaluates on window-shown and once ~150ms after input-view start so editors that answer text queries late cannot leave it hidden until a selection change.
